...
class SK_ahliwaris extends CI_Controller {

	function __construct(){
        parent::__construct();
		$this->load->model('m_sk_ahliwaris');
	...
	public function index()
	{
		...
		$this->load->view('surat/sk_ahliwaris');
    ...
    //    $pdf->Cell(190,7,'__________________________________________________________________________________',0,1,'C');        
        ...
        $pdf->Cell(190,7,'SURAT KETERANGAN AHLI WARIS',0,1,'C');
        $pdf->Cell(190,7,'Nomor : 520/02/404.319.04/2019',0,1,'C');        
        ...
        $pdf->Cell(10,6,'',0,1);
        $pdf->Cell(140,7,'Orang tersebut diatas adalah Ahli Waris dari Almarhum :',0,1,'C');
        $pdf->Cell(10,6,'',0,1);
        $pdf->Cell(42,6,'Nama                         :',0,0,'');
        $pdf->Cell(42,6,$row->nama_lengkap,0,1);
        $pdf->Cell(42,6,'Tempat Tgl. Lahir     :',0,0,'');
        ...
        $pdf->Cell(20,6,'Dsn. '.$row->dusun.', Rt/Rw '.$row->rt.'/'.$row->rw.', Desa Sidorejo',0,1);
        $pdf->Cell(10,6,'                                        Kec. Geneng, Kab. Ngawi',0,1);
        $pdf->Cell(140,7,'Demikian surat Keterangan Ahli Waris ini diberikan kepada yang bersangkutan untuk',0,1,'C');
        $pdf->Cell(58,7,'dipergunakan sebagaimana mestinya.',0,1,'C');
        $pdf->Cell(10,7,'',0,1);
    ...
    function get_warga(){
        ...
        $data=$this->m_sk_ahliwaris->get_data_warga_bynik($nik);
        echo json_encode($data);
    }

    function redirectLaporan(){
    	redirect('SK_ahliwaris/suratpdf?nik='.$this->input->post('nik2'));
    }
