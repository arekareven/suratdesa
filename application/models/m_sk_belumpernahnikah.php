    ...
    function get_no_invoice(){
        ...
        $query = $this->db->query($sql);
		  if($query->num_rows() > 0){      
			  ...
			  $kodetampil = "455.2/".$no."/404.319.04/".$tgl;  //format kode
			  return $kodetampil;
    }
...
/*    function get_no_invoice(){
        $this->db->select('RIGHT(surat.no_surat,2) as no_surat', FALSE);
		 $this->db->order_by('no_surat','DESC');    
          $this->db->limit(1);    
         //$query=$this->db->query("SELECT no_surat FROM surat WHERE nama_surat='Surat Keterangan Belum Nikah' order by no_surat desc");
		  ...
		  if($query->num_rows() <> 0){      
              ...
              $tglb=date('m'); 
			  $batas = str_pad($kode, 3, "0", STR_PAD_LEFT);    
			  $kodetampil = "474.2"."/".$tglb.".".$batas."/404.319.04/".$tgl;  //format kode
			  return $kodetampil;
    }*/
}